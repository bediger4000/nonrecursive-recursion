...
main(int ac, char **av)
{
		 ...
		 */
		printf(
	   "main = %p, copyup %p, memcpy %p, printf %p, malloc %pm free %p, size %ld\n",
	   main, copyup, memcpy, printf, malloc, free, (size_t)f2 - (size_t)copyup);
		...
		j = copyup(1, i, printf, copyup, memcpy, (size_t)f2 - (size_t)copyup);
...
)
{
		...
		k = (*fptr)(i + 1, j, xptr, (void *)fptr, bptr, size);
